...
class JPEGCompress(object):
    ...
    def dct_and_quant(self, img_path):
        ...
        img_blocks = [
            np.hsplit(item, width // 8) for item in np.vsplit(img_arr, height // 8)
        ]
        dct_blocks = [
    ...
    def stegano(self, img_path, quant_blocks):
        ...
        for i in bin_seq:
            print(i)
        f = open(file='123.txt', mode='rb')
        filef = f.read()
        bin_seq = "".join([bin(str)[2:]for str in filef])
        bin_seq = deque([int(c) for c in bin_seq])
    ...
    def reverse_process(self, stegano_blocks):
        ...
        iquant_blocks = [
            [block * quant_tbl for block in line] for line in stegano_blocks
        ]
        idct_blocks = [
        ...
        stack_arr = np.array(
            np.vstack([np.hstack(line) for line in idct_blocks]), dtype=np.uint8
        )
    ...
    def decompress_pic(self, codec, compress_pic):
        ...
        arrays = [
            self.sequence2mat(decoded[i * 64 : i * 64 + 64])
            for i in range(len(decoded) // 64)
        ]
        decoded_blocks = [
            arrays[i * 64 : i * 64 + 64] for i in range(len(arrays) // 64)
        ]
                    ...
                    for m in range(height):
                        for n in range(width):
                            if block[m][n]:
                                ...
                                if (
                                    len(bin_seq)
                                    == 8 * secret_shape[0] * secret_shape[1]
                                ):
                                    raise ExitLoop
        except ExitLoop:
            bin_seq = np.split(np.array(bin_seq), len(bin_seq) // 8)
            img_arr = np.array(
                [int("".join(item), 2) for item in bin_seq], dtype=np.uint8
            ).reshape(secret_shape)
...
if __name__ == "__main__":
    ...
    Image.open("secret_original.jpg").convert("L").resize(
        secret_shape, Image.ANTIALIAS
    ).save("secret_grey.jpg")
    ...
    handler.display_result(
        "basic_grey.bmp", "reversed_img.bmp", "secret_grey.jpg", "extract_img.jpg"
    )
