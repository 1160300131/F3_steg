...
class JPEGCompress(object):
    ...
    def reverse_process(self, stegano_blocks):
        ...
        for i in range(3):
            quant_tbl = quant_tbl_1 if i == 0 else quant_tbl_2
            iquant_blocks = [
                [block * quant_tbl for block in line] for line in stegano_blocks[i]
            ]
            idct_blocks = [
...
if __name__ == "__main__":
    secret_shape = (80, 80)
    Image.open("secret_original.jpg").resize(secret_shape, Image.ANTIALIAS).save(
        "secret_colorful.jpg"
    )

    handler = JPEGCompress()
    codec, encoded = handler.stegano_compress(
        "basic_colorful.bmp", "secret_colorful.jpg"
    )
